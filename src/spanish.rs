...
static a_4: &'static [Among; 3] = &[
    Among("able", -1, 1, None),
    Among("ible", -1, 1, None),
    Among("ante", -1, 1, None)
];

static a_5: &'static [Among; 3] = &[
    Among("ic", -1, 1, None),
    Among("abil", -1, 1, None),
    Among("iv", -1, 1, None)
];

static a_6: &'static [Among; 46] = &[
    Among("ica", -1, 1, None),
...
fn __r_attached_pronoun(env: &mut SnowballEnv, context: &mut Context) -> bool{
    // (, line 67
    // [, line 68
    env.ket = env.cursor;
    // substring, line 68
    if env.find_among_b(a_1) == 0{
        return false;
    }
    // ], line 68
    env.bra = env.cursor;
...
fn __r_y_verb_suffix(env: &mut SnowballEnv, context: &mut Context) -> bool{
    // (, line 167
    // setlimit, line 168
    let v_1 = env.limit - env.cursor;
    // tomark, line 168
...
fn __r_residual_suffix(env: &mut SnowballEnv, context: &mut Context) -> bool{
    ...
    env.ket = env.cursor;
    // substring, line 205
    let among_var = env.find_among_b(a_9);
    if among_var == 0{
        return false;
    }
    // ], line 205
...
pub fn _stem(env: &mut SnowballEnv) -> bool{
    ...
    'lab1: loop{
        ...
        if !__r_attached_pronoun(env, context){
             break 'lab1;
        }
        break 'lab1;
    }
    ...
    'lab2: loop{
        ...
        'lab3: loop{
            ...
            'lab5: loop{
                ...
                if !__r_y_verb_suffix(env, context){
                     break 'lab5;
                }
                break 'lab3;
                break 'lab5;
